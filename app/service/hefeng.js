...
class HefengService extends Service {
  ...
  async getLocationId(longitude, latitude) {
    ...
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 城市信息查询, location=${lng},${lat}`)
  ...
  /**
   * 用于在控制器中，处理和风天气相关接口，输出可供当前文件方法直接使用的参数
   * @since 2021-03-17
   *
   * @tag [Controller]
   *
   ...
   */
  async handleControllerParams(type) {
    if (!type) {
      throw new Error('未指定 type')
    }
    ...
    /** 经纬度保留到小数点后的位数，，目前 2 位已经够了，偏差值 1.4km 内 */
    const decimal = 2
  ...
  async weatherNow(location) {
    ...
    const { data: resData } = await app.axios(requestOptions)
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 实时天气, location=${location}`)
      app.redis.set(redisKey, JSON.stringify(resData), 'EX', timeout)
      return resData
    } else {
   ...
   */
  async fore7d(location) {
    const { app, service, logger } = this
    const { key: redisKey, timeout } = service.keys.hefengFore7d(location)
    const cacheResult = await app.redis.get(redisKey)
    if (cacheResult) {
      return JSON.parse(cacheResult)
    }
    ...
    const { data: resData } = await app.axios(requestOptions)
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 逐天天气预报（7天）, location=${location}`)
      app.redis.set(redisKey, JSON.stringify(resData), 'EX', timeout)
      return resData
    } else {
      logger.error(`[接口请求错误] 和风天气 - 逐天天气预报（7天）, 请求参数 params=${JSON.stringify(requestOptions.params)}, 响应 code=${resData.code}`)
    }
  }
  ...
  async fore15d(location) {
    ...
    const { data: resData } = await app.axios(requestOptions)
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 逐天天气预报（15天）, location=${location}`)
      app.redis.set(redisKey, JSON.stringify(resData), 'EX', timeout)
      return resData
    } else {
  ...
  async fore24h(location) {
    ...
    const { data: resData } = await app.axios(requestOptions)
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 逐小时天气预报（24小时）, location=${location}`)
      app.redis.set(redisKey, JSON.stringify(resData), 'EX', timeout)
      return resData
    } else {
  ...
  async minutelyRain(location) {
    ...
    const { data: resData } = await app.axios(requestOptions)
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 分钟级降水, location=${location}`)
      app.redis.set(redisKey, JSON.stringify(resData), 'EX', timeout)
      return resData
    } else {
  ...
  async indices(location) {
    ...
    const { data: resData } = await app.axios(requestOptions)
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 天气生活指数, location=${location}`)
      app.redis.set(redisKey, JSON.stringify(resData), 'EX', timeout)
      return resData
    } else {
  ...
  async airNow(location) {
    ...
    const { data: resData } = await app.axios(requestOptions)
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 实时空气质量, location=${location}`)
      app.redis.set(redisKey, JSON.stringify(resData), 'EX', timeout)
      return resData
    } else {
  ...
  async air5d(location) {
    ...
    const { data: resData } = await app.axios(requestOptions)
    if (parseInt(resData.code, 10) === 200) {
      logger.info(`[接口请求成功] 和风天气 - 空气质量预报（5天）, location=${location}`)
      app.redis.set(redisKey, JSON.stringify(resData), 'EX', timeout)
      return resData
    } else {
